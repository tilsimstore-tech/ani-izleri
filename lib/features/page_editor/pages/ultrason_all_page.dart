...
class UltrasonAllPage extends StatefulWidget {
  final AppTheme t;
  final Map<String, dynamic> entry;
  final ValueChanged<Map<String, dynamic>> onChanged;

  const UltrasonAllPage({super.key, required this.t, required this.entry, required this.onChanged});

  @override
  State<UltrasonAllPage> createState() => _UltrasonAllPageState();
}

class _UltrasonAllPageState extends State<UltrasonAllPage> {
  ...
  void initState() {
    super.initState();
    final raw = (widget.entry['items'] as List?) ?? [];
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        if (_picking) ...[
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Text('Hangi aya ait?', style: TextStyle(fontSize: 13, color: t.soft)),
        ...
        const SizedBox(height: 14),
        if (_items.isEmpty)
          EmptyState(t: t, text: 'Ay seç, fotoğrafı ekle — otomatik sıralanır'),
        ...presentMonths.map((m) {
